...
export async function handleResultStarmax(data: any) {
    let conn;
    ...
    try {
        ...
        if (requestdetlistData.length > 0) {
            ...
            for (const detlist of requestdetlistData) {
                ...
                if (macInData.length > 0) {
                    ...
                    if (["UHK17", "UHK18", "UHK19", "UHK20", "UHK21"].includes(testCodedet)) {
        ...
        console.error("Error in handleResultStarmax:", error);
        throw new Error(error.message || "Failed to handle Starmax result");
    }
}
